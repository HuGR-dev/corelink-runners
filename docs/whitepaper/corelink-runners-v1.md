...
> **Status:** v1 · 2026-06-09 · **canonical product vision.**
> The single source of truth for *why CoreLink Runners exists and what it must be.*
...
> Author: HuGR techlead. Family: **HuGR → CoreLink → { Cache · Runners · Workspaces } ; hugit.**
...
how CoreLink's cache — already its moat — becomes compute revenue, and the execution
substrate beneath hugit, the HuGR forge for agent fleets. **We don't sell a cheaper minute.
...
| **Build-tool native** | Bazel-class remote-execution on the cache; slots under existing tooling and under hugit. |
...
tenancy, and PAT auth. It is itself consumed by **hugit** (its anchor tenant) and by **CoreLink
Workspaces** (agent sandboxes / dev boxes are workspace SKUs that run on this fabric).
...
requirement in the system, and the reason the seam with hugit is frozen (`docs/spec/`).
...
We never bill as though a memoized result re-ran, and we never expose a per-minute meter (least
of all through hugit's flat plan). The value sold is the cache + the predictability — **not a
...
6. **One product, one bill (downstream).** hugit customers see only hugit; Runners is invisible COGS there, and a first-class product directly.
...
- **M0 — Spec lock** *(now).* Freeze hugit's consumed contract; the CoreLink techlead fills the
...
memo; mitigated by controlling the knobs + hugit's non-determinism detection. (b) *Isolation
...
provenance — those are hugit's. It does not reimplement the cache — that is CoreLink's. It does
...
often elidable; a **compute** layer that turns that into a flat, predictable, cheaper-and-faster
product; and a **forge** (hugit) that is both the flagship consumer and a second front door —
all on one content-addressed substrate, billed flat, **improving with scale.** Per-minute vendors
