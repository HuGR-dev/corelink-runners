...
the bounded Worker/native ACK command list. Workflow-level concurrency keys on
repository, pack ID, PR number, candidate SHA, and target base SHA with
`cancel-in-progress: false`, so duplicate dispatch identities are serialized
without cancelling an active proof. A dispatcher selftest guards each key
dimension and the no-cancellation setting.
