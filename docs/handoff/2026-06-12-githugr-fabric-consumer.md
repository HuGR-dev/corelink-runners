...
abaixo, provado verde nesta mesma máquina em 2026-06-12 (o handoff anterior
`2026-06-12-githugr-runner-response.md` também recomendou fixar as VERSÕES das
tools no bloco `with:`, p.ex. `cargo-audit@0.22.2`, `cargo-deny@0.19.8`, para
evitar que `@latest` quebre num re-publish de asset — então o fix completo é a
troca do SHA mais esse hardening de versões):
...
**(a)** Apliquem o fix no `ci.yml` (`taiki-e/install-action@7a79fe8c…` — detalhe
acima e no handoff anterior, incluindo o hardening de versões das tools no bloco
`with:`) para desbloquear o runner atual.
