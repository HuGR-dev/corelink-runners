...
**Status:** DECISÃO PENDENTE — bloqueio de identidade do assinante
...
(única opção que não altera o `LeaseResponse` congelado). Se a arquitetura do
...
|---|---|---|---|---|
| **A — mesmo PAT** (default atual) | O hook é registrado com o PAT do acquire. O assinante precisa apresentar o mesmo PAT. | Nenhuma — `LeaseResponse` congelado intacto. | Zero atrito; funciona se hugit usa o tenant PAT nas duas pontas. | Falha silenciosa (503) se hugit separa papéis. |
...
- A suposição está **flaggeada em código** no site de registro
