...
- Decisão arquitetural 2026-09-08: Hugit/githugr são projetos externos desacoplados e não fazem parte do produto, ambiente, autoridade ou aceite do CoreLink. O incidente interno de exposição de secret continua registrado, mas a rotação v2/controller não dependem de manifesto Hugit nem de inventário Hugit. O único pré-requisito operacional é o manifesto do `corelink-fabricd`/Cloudflare e o inventário das instâncias CoreLink ativas; não registrar valores de segredo. Ver `docs/adr/0014-corelink-standalone-product-boundary.md`.
