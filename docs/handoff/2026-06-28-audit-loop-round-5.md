...
| 3 | med | `repo_allowlist` (defense-in-depth) enforced on the webhook path but **absent on `/v1/leases`** (`leases.rs`). | **DESIGN-HANDOFF** (`2026-06-28-DESIGN-runner-scope-tenant-binding.md`, with #2) — same runner-acquire-authz surface; **latent** (single-tenant now). The cheap "mirror the webhook allowlist on the acquire path" option is specified there for the M2 wave. |
...
| 11 | low | `ClwRunSpec` fields have no upstream charset validation (shell-injection is held only by `shell_join` quoting — Inv-4 HOLDS, but defense-in-depth) (`clw`). | **NOTE (not a defect)** — the injection invariant HOLDS (`shell_join` quotes every field); upstream charset validation is belt-and-suspenders on an already-safe path. Specified in the design-handoff as an optional moat-hardening; deferred (no exploitable gap). |
