...
(lease concorrente ativo). O fabric **define** os tipos de metering de slot
(`SlotOccupancyEvent`, `SlotMeter` com pico por tenant), com testes unitários em
`corelink-fabric` — mas a **emissão** desses eventos pelos handlers de
acquire/close **não está cabeada ainda**: nenhum call-site nos handlers publica
`SlotOccupancyEvent`. Essa emissão pelos handlers é uma peça adicional, prévia e
separada do wiring ao endpoint de billing. O que falta, em ordem: (1) cabrear a
emissão dos handlers, (2) ligar ao Stripe de vocês em vez de subir uma segunda
integração.
...
2. Como o plano comprado chega até nós como source of truth para o cap —
   **push** (webhook em plan_change) ou **pull** (consultamos no acquire)?
   O nosso `PlanRegistry::set_plan` é in-memory apenas (sem transporte push/pull
   implementado ainda — é aspiracional); precisamos saber qual mecanismo vocês
   suportam para definirmos a impl concreta no wiring M2.
...
> **Nota para o techlead — mismatch de ladder de planos a reconciliar:**
> `docs/product/pricing.md` define os tiers Starter/Pro/Team/Scale/Max (caps de
> concorrência 20/40/80/160/320). O enum `PlanTier` no código
> (`crates/corelink-fabric/src/plans.rs`) usa uma ladder diferente: Free/Solo/Team/Scale
> (caps 1/1/4/12). O campo `plan` retornado na introspection response (seção 3)
> precisa que as duas nomenclaturas sejam reconciliadas antes do wiring M1/M2 —
> confirmar qual vocabulary de tier a integração usará.
...
| `CoreLinkPlanStore`: impl do trait `PlanSource` que consulta (pull) ou recebe (push) o plano do tenant e popula `max_concurrency` / `rate_ceiling_per_min` | Atrás de `FABRIC_PLANS_BACKEND=corelink` (default: `static`); substitui `StaticPlans` |
| Reporter de slot-occupancy + vCPU-h ceiling | Os tipos `SlotOccupancyEvent`/`SlotMeter` existem e têm testes; a emissão pelos handlers de acquire/close ainda precisa ser cabeada, e só então a wiring ao endpoint de billing de vocês é acrescentada |
