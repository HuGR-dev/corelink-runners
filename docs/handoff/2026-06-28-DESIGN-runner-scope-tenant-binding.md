...
## Round-8 red-team confirmation + a new CAS-tenant-confusion facet
The round-8 red-team (chaining surfaces, attacker holds a valid PAT) re-confirmed the #2 RunnerScope chain end-to-end as **high**, and surfaced a coupled facet: when the box is minted onto the victim's repo (per #2) AND `cas_pat_mint` is configured, `finalize_admitted_lease` mints the CAS PAT with `owner_tenant = the ATTACKER's tenant`, and `inject_clw_env` writes `CLW_TENANT=<attacker>` / `CLW_TOKEN=<attacker's PAT>` into the box. So the victim's CI (running on the attacker-minted runner) performs its CAS/AC reads+writes in the **attacker's** namespace — a cross-tenant data-confusion on top of the unauthorized runner mint. **Same root + same preconditions** (latent until M2 multi-tenant; the App must be installed on the victim repo). The fix is the same: bind the RunnerScope (and therefore the mint's `owner_tenant`) to a per-tenant repo entitlement. Confirmed in `2026-06-28-audit-loop-round-8-redteam.md` (#1, #2).
