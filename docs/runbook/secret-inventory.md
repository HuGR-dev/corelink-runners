...
| `GITHUB_MINT_TOKEN` | First-party JIT runner mint (`generate-jitconfig`), `Administration:write` on `HumanGuardrail` repos. | Absent ⇒ `/webhook` 503. | Dogfood path. Customer repos use the App path instead. |
