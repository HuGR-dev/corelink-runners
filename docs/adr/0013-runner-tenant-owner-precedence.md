...
When a GitHub installation is supplied, its server-side installation-to-tenant
mapping is the owner of record. A configured acquiring PAT is an additional
credential; it cannot replace or hide that installation. If both identities are
...
| Repository explicitly maps a PAT secret, but that secret is missing/blank | Unavailable | Worker preparation refuses before authorization, slot, mint, claim or provider effects; no installation fallback. |
