...
3. **Track-C stacks on top as defense-in-depth on the software boundary — CF-path status: PARTIAL.** The isolation boundary IS the per-lease Firecracker microVM; Track-C is the additional software boundary layered on top. Its coverage on the CF substrate is honestly **partial** — some controls are deployed, others remain follow-ups (they degrade defense-in-depth, not the microVM boundary):
   - **Deployed on the CF path:**
     ...
     - **Operator egress kill-switch** — `/v1/egress-cutoff` cuts all outbound egress at runtime (SDK `setDeniedHosts` + `"*"` catch-all, which DOES glob-match every host) without a full `destroy()`.
     ...
     - **App-layer resource caps (ulimit)** — the runner image bounds the untrusted job with `ulimit -u` (fork-bomb / pids) and a non-root `USER`, the in-image equivalent of the on-box `--cap-drop`/`--pids-limit`. (Memory is NOT app-layer-cappable on this substrate — `ulimit -v` is virtual-AS and breaks real jobs, `RLIMIT_RSS` is a Linux no-op, and CF exposes no per-container cgroup memory knob; memory containment is the per-lease microVM envelope + in-VM OOM-killer.)
   - **Best-effort, NOT-yet-verified (G2 metadata/link-local egress — NOT CLOSED on CF by this mechanism):** the `deniedHosts` metadata denylist is only a PARTIAL, unverified control. The CF SDK matches `deniedHosts` with `simpleGlobMatch` — pure literal / `*`-glob string matching with **no CIDR math** — so the link-local CIDR ranges (`169.254.0.0/16`, `fe80::/10`, `fd00::/8`) are **inert** (they never match), and only the exact hosts `169.254.169.254` and `metadata.google.internal` match, and only for egress that traverses the SDK proxy (**raw sockets bypass it entirely**). **G2 (metadata / link-local egress block) is therefore NOT closed on the CF path by this mechanism.** Real IMDS/link-local blocking needs platform-network-layer filtering (outside this Worker/SDK), and even the exact-host entries still owe a **live-account smoke** before they can be trusted.
   - **Follow-ups (NOT yet on the CF path):** the G2 platform-network-layer metadata/link-local filter (above), env-0 cred-ticket rollout (C2c), per-tenant repo allowlist, tenant-suspend enforcement, and the read-only-rootfs / no-new-privileges cap-drop set that the on-box `DockerEngine` applies but the CF substrate does not.
...
3. **Track-C stays ON in production — CF coverage is PARTIAL (not accept-with-waiver).** The software hardening is mandatory even under a strong sandbox (defense-in-depth), never treated as redundant. Its CF-path coverage is enumerated honestly in Why-3: **deployed** = image pin, revoke-on-complete, egress kill-switch, required exec-server auth, and app-layer ulimit (pids) cap; **best-effort but NOT verified / NOT closing G2** = the `deniedHosts` metadata denylist (exact-host only, no CIDR match, raw-socket bypass — G2 metadata/link-local egress is NOT closed on CF by this mechanism); **follow-ups** = the G2 platform-network-layer filter, env-0 cred-ticket, repo allowlist, tenant-suspend, read-only-rootfs/cap-drop. The isolation boundary remains the per-lease Firecracker microVM regardless of which follow-ups land — the caps/egress/exec fixes are BUILT, so this is a stated-partial posture, not a waiver granted against unbuilt work.
