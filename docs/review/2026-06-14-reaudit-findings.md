...
- [x] **P1 → DECIDED (documented scope + FC3, not a code bug)** `handlers/close.rs:318` — close signs the `AttestationChain` over client-supplied tree/def/runner axes. **Techlead ruling:** at M1 the fabric does not independently re-derive the input axes from content (that is FC3, content-addressed resolution — a documented non-goal per `exec.rs`); the memo_key gate (W2-C) makes the axes self-CONSISTENT, v2 binds the OUTCOME, and the fence/X4 bound the runner. The §7 attestation CLAIM scope is documented explicitly + routed to hugit: `docs/handoff/2026-06-14-hugit-attestation-claim-scope.md`. Upgrades to fabric-observed axes when FC2/FC3 land.
