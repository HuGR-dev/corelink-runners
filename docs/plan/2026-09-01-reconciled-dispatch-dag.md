# Reconciled dispatch DAG — rev6 Round-9 repair draft
...
dispatcher must reference this file and must not restate its DAG. The current Round-9 repair tree
...
not claim application behavior or a deployment. T6-W15 alone owns integration, deployed-version
...
scheduler, state, delivery and credential domains; T6-W15 then binds, deploys and proves them, and
must be live before T1-W6 may re-arm PG.
...
their freshness. It reopens and integrates the base's index, scheduler, config, state, incident and
type seams, applies the provider-cursor migration, redeploys the external service, and proves that
stale or frozen provider watermarks fail closed; provider/correlator source files alone cannot seal
this phase. `T6-W14` later makes canary read the passive fabricd outer-Worker lifecycle route
...
`T6-W4` durably enqueues each scheduled tick before transmission through a Durable Object outbox;
its Wrangler binding/migration and crash/retry/order tests are mandatory atoms, not implied by an
envelope unit test. `T1-W6` similarly has two non-interchangeable write-only producer scopes
(`fabric-server` and `fabricd-proxy`), each with its own durable ordered outbox, key id and credential
epoch; neither may reuse any read-only O-CFINVENTORY principal. T1-W6's provider inactivity proof
therefore names O-CFINVENTORY directly and uses only the rearm-probe principal.
...
| T6-W12 | W3 live proof | T6-W15, T6-W9, T1-W6, T3-W16, O-CFINVENTORY, T7-W4b | `deploy/cost-monitor/Containerfile`; `deploy/cost-monitor/config.schema.json`; `deploy/cost-monitor/package.json`; `deploy/cost-monitor/package-lock.json`; `deploy/cost-monitor/src/index.ts`; `deploy/cost-monitor/src/scheduler.ts`; `deploy/cost-monitor/src/state.ts`; `deploy/cost-monitor/src/incidents.ts`; `deploy/cost-monitor/src/types.ts`; `deploy/cost-monitor/src/provider.ts`; `deploy/cost-monitor/src/correlator.ts`; `deploy/cost-monitor/src/capability_rules.ts`; `deploy/cost-monitor/src/synthetic_ingest.ts`; `deploy/cost-monitor/migrations/0002-provider-cursors.json`; `deploy/cost-monitor/test/provider.test.ts`; `deploy/cost-monitor/test/provider-stale-frozen.test.ts`; `deploy/cost-monitor/test/correlator.test.ts`; `deploy/cost-monitor/test/cursor-crash.test.ts`; `deploy/cost-monitor/test/provider-unavailable.test.ts`; `deploy/cost-monitor/test/incident-boundary.test.ts`; `deploy/cost-monitor/test/acked-incident-update.test.ts`; `deploy/cost-monitor/test/recovery-horizon.test.ts`; `deploy/cost-monitor/test/c1-c5-rules.test.ts`; `deploy/cost-monitor/test/c1-c5-synthetic-ingest.test.ts`; `docs/plan/evidence/T6-W12-independent-monitor.json` | Sol / alerting |
...
B03: T1-W5 T2-W2a T3-W18 T9-W1
B04: T2-W2b T4-W1 T5-W6 T6-W4
B05: T2-W3 T2-W4 T2-W6 T4-W2 T6-W15 T6-W9
B06: T1-W6 T3-W3 T6-W13
B07: T1-W2 T1-W3 T2-W5 T3-W1 T3-W7 T4-W7 T4-W8 T5-W4
B08: T3-W2 T3-W8 T5-W5 T6-W11 T6-W5 T6-W7 T7-W5 T8-W7
B09: T8-W1
B10: T8-W3
B11: T3-W14
B12: T3-W9
