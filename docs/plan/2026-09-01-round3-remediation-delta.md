...
latch cannot auto-clear: only complete T6-W12 candidate/cutover/active-final reproof, exact T1-W6
tuple rebinding and T1-W6's atomic consumption of a fresh, unexpired, one-shot `O-PG-REARM`
authorization bound to the final tuple/scans/poll and exact flag transition may clear it.
...
| new **T3-W17** | A3.30 repo/test half | 1 phase | worker intake/re-drive switches, durable drain + focused tests | T0-W1; this is the deterministic **test owner** | W0 containment implementation. Its frozen implementation contract is `docs/plan/contracts/T3-W17.md`: independent exact-value switches, SQLite-backed singleton containment authority, durable pause before 202, unavailable authority returning 503, and ordered/idempotent resume across every named crash point. It cannot seal A3.30 without T3-W18. |
