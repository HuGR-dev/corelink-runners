...
the lead installs a read-only provider credential that can list applications, all paginated
instances, immutable start events and cumulative active-minute/usage counters, but cannot create,
...
**O-MONITORHOST (reserved obstacle).** Before T6-W15 implementation or any external-monitor probe,
the owner selects and records a **non-Cloudflare** compute/runtime and scheduler, durable incident and
cursor store, and alert-delivery transport. The version-bound artifact
`docs/plan/evidence/O-MONITORHOST-external-monitor.json` names each selected service, region and
account/failure domain; proves the scheduler's 60 s upper interval, each complete scan/correlation
processing budget ≤30 s and delivery budget ≤30 s in 3/3 injections; and records separately scoped
credential domains for
provider inventory read, each event producer's write-only ingestion, monitor state, and alert
delivery. None may reuse `O-CFTOKEN`, a Cloudflare account-wide API key, the monitored Workers/DOs,
or Cloudflare compute/durable state/delivery. The producer credentials are scoped to exactly one
`(source, service/application)` and carry `key_id` plus monotonic credential epoch; the artifact
proves overlap rotation, old-key revocation, replay/sequence-regression refusal and cross-source /
cross-application refusal without exposing key material. The selected durable store must atomically
commit incident state with an outbox delivery id, and the selected delivery path must provide a
version-bound idempotency capability: a byte-identical retry with that id returns the same durable
acknowledgement and emits no second page, while id reuse with different bytes is refused. Crash
fixtures before enqueue, after enqueue, after delivery and before acknowledgement are lossless and
emit one page. Killing fabricd, spawn-worker, canary and
the entire Cloudflare request path must still let the external scheduler/state/delivery issue the
missing-source or provider-unavailable page in 3/3 trials. If a concrete runtime, store, delivery
path, durable-outbox/idempotency capability, permission schema, rotation/revocation procedure or
independence proof is absent, the obstacle remains unresolved, deployment is refused and A6.10/A6.20
are RED; no implementer may invent a vendor or substitute `deploy/cloudflare-cost-monitor/**`.
...
| A6.10 | T6-W4 owns only the canary's durably queued authenticated scheduled-tick producer and `deploy/cloudflare-canary/test/scheduled-tick-envelope.test.ts`; it earns no A6.10 credit. Existing/principal T6-W15 owns the deployed external detector and `deploy/cost-monitor/test/canary-missing-tick.test.ts`. The detector, not the canary, tracks the expected 5-minute schedule. A missing tick pages within 120 s of its `scheduled_for` (`60 s` scheduler bound + `30 s` processing + `30 s` delivery) in 3/3 stopped-canary injections; killing the canary cannot kill detector state or delivery. Only the T6-W15 external proof can green A6.10. |
...
| **A1.11** | test+probe | new **T1-W6** (1 total) | fabric server/proxy PG ledger + exporter init, persistent breaker, producer outbox and evidence | D12 · T1-W5 · T6-W15 | INV-3, INV-4, INV-5 | T6-W15's external base must be deployed and its ingress/state/delivery proof green before PG rearm. Every PG/exporter connection first reads the durable breaker authority, wins a CAS attempt permit that stores `(epoch, in_flight, reserved_failure_level, reserved_failure_deadline)`, and durably enqueues the signed attempt/breaker envelope before any socket/init call. Unreadable authority, failed permit write or failed envelope/outbox commit yields 0 PG attempts and typed 503 across 100 concurrent demands and after restart; no process-memory fallback exists. At t0 with healthy authority, 100 demands singleflight to exactly 1 permit/attempt/refusal, reserving durable backoff of 1, 2, 4, 8, 16 then 32 min capped. For each level, restart at `next_attempt_at-1s` and exactly at it yields respectively 0 and exactly 1 attempt. A failed attempt finalizes the reserved next tuple; if finalization is write-failed, the durable in-flight row remains fail-closed and restart honors its already-reserved tuple. A 32-minute failure reserves a new `+32 min` deadline. Diagnostics/readiness/timer/cron cause 0. One successful permitted half-open closes it | Red: PG is rearmed before the independent monitor, ledger/exporter failure occurs pre-bind, producer evidence can be lost before the socket, breaker storage failure falls back to memory/PG, periodic traffic sustains reconnect burn, or a failed half-open leaves the old tuple observable. Green: unreadable-read, failed-permit-write and failed-outbox-commit fixtures prove 0 PG attempts before and after restart; failed-finalization fixtures prove no second attempt and recovery from the reserved tuple. Ledger/exporter refusal fixtures bind diagnostics in 10/10 starts, keep readiness/mutation at 503, create 0 degraded rows and pass all restart boundaries. After one failed demand, 16 request-free minutes make 0 attempts; provider scans at minutes 6, 11 and 16 report fabricd inactive 3/3. T1-W6 cannot start until T6-W15 has deployed the external receiving and delivery path. |
| **A6.20** | test+probe | base owner: principal **T6-W15**; provider/live owner: new **T6-W12** (1 item total) | base provider-neutral OCI service, ingestion/scheduler/state/outbox/delivery and missing-source detectors; follow-on provider adapter, correlator and live evidence | base: T6-W15 after T6-W4 · O-MONITORHOST · T7-W4b; provider/live: T6-W12 after T6-W15 · T1-W6 · T3-W16 · O-CFINVENTORY · T7-W4b | INV-3, INV-5 | **Base phase:** T6-W15 deploys `deploy/cost-monitor/**` outside Cloudflare, proves the external A6.10 detector, and freezes ingestion, credential, incident, durable-outbox and delivery contracts before T1-W6 rearm. Every producer first persists the complete signed envelope `(kind, source, service, application, event_id, producer_seq, occurred_at, scheduled_for, version, key_id, credential_epoch, payload_digest)` in its durable ordered outbox, then sends those exact bytes; a later envelope cannot overtake an unacknowledged one. The monitor returns an acknowledgement only after its CAS ingest commit. A byte-identical duplicate returns the same durable acknowledgement with no state/page effect; divergent reuse of an id/sequence, lower sequence, stale/future data, revoked epoch or cross-source/application use is refused. **Provider/live phase:** O-CFINVENTORY proves provider maximum source age 120 s; O-MONITORHOST proves external poll/scheduler interval ≤60 s, processing ≤30 s and delivery ≤30 s, while each producer packet proves its ordered outbox retry interval ≤60 s. Breaker-open push pages within **120 s** of `occurred_at` (`60 + 30 + 30`); provider-unavailable/partial pages within **120 s** of the failed scheduled scan (`60 + 30 + 30`); missing lifecycle sample or canary tick pages within **120 s** of `scheduled_for` (`60 + 30 + 30`); a one-scan provider condition pages within **240 s** of occurrence (`120 + 60 + 30 + 30`); and running-over-binding in two complete scans 60 s apart pages within **330 s** (`120 + 60 + 30 + 60 + 30 + 30`), all in 3/3 injections. T6-W12 fully paginates before cursor commit, overlaps 2 min and deduplicates immutable event ids; partial/429/error preserves the old cursor and is never absence-as-zero. Every output—breaker-open; unmatched start, active-minute or billed-usage delta; running over durable bindings; active >6 min with 0 authenticated demands; missing lifecycle/tick; or provider unavailable/partial—shares one durable CAS `open_incident_id` per `(service/application)` with `OPEN`, `ACKED`, `RECOVERING`, `CLOSED`. First all-clear captures an all-source high-water map and recovery candidate; recovery requires every complete source high-water to advance beyond that candidate and at least **330 s** continuously quiet. Any signal, missing/partial source, failed scan or sequence gap cancels recovery. Exactly 1 initial page, at most 1 unacknowledged escalation at 5 min and exactly 1 recovery atomically close/clear the pointer; clock boundaries never change an open id | Red: the base is absent before PG rearm, either phase claims partial green, producer bytes are not durable before send, an ACK precedes commit, retry changes an ACK/effect, a short-lived box disappears between scans, a partial page advances state, recovery closes before all-source high-water + 330 s quiet, a 15-minute boundary splits an incident, or any signal opens a separate stream. Green: T6-W15's `outbox-recovery`, `delivery-dedupe`, `ingest-idempotency`, lifecycle/canary missing-source and credential-isolation fixtures pass against the deployed external base; then T6-W12's provider/correlator, cursor-crash, incident-boundary and `recovery-horizon` fixtures plus version-bound live evidence pass. Killing each monitored component and the Cloudflare path still delivers 3/3 initial pages with the shared ack/escalation/recovery matrix. T6-W15 alone cannot green A6.20; T6-W12 cannot run without the base and both mandatory phases are required. Later T6-W10 consumes this external primitive. |
...
| new **T3-W16** | A3.31 | 1 | worker durable attempt/DO-handle binding, cancellation barrier, monitor producer outbox + read-only provider inventory evidence | T8-W3 · T3-W18 · T2-W2b · O-CFINVENTORY | Pre-start durable handle binding; tombstone, provider-proven linearizable exact-handle cancellation and two post-barrier confirmed-down reads precede supersession. Durable attempt/binding envelopes commit before send and retry in order from `deploy/cloudflare/test/attempt-monitor-outbox.test.ts`. No guarantee means indefinite refusal. Provider inventory is read-only cost/cross-check evidence, never destructive eligibility; ambiguous/unjoined active rows pause replacement. |
...
| existing **T6-W15** | A6.10 + A6.20 base half | 1 item + 1 phase | provider-neutral external base: Containerfile/config, ingress, lifecycle/tick detectors, scheduler, state, outbox, delivery, types and base evidence | T6-W4 · O-MONITORHOST · T7-W4b | This newly introduced identifier is registered as the principal suite's 48th WP, not as staged-new. W1 deploys `deploy/cost-monitor/**` outside Cloudflare and alone earns A6.10. Exact base tests include `canary-missing-tick.test.ts`, `lifecycle-missing.test.ts`, `credential-isolation.test.ts`, `outbox-recovery.test.ts`, `delivery-dedupe.test.ts` and `ingest-idempotency.test.ts`. The base must be green before T1-W6; it cannot seal A6.20 without T6-W12. |
| new **T1-W6** | A1.11 | 1 | fabric PG ledger/exporter init + breaker producer outbox/evidence | D12 · T1-W5 · T6-W15 | PG only; after the external base is green, diagnostics-first demand-only CAS permits and `monitor_outbox.rs` persist signed breaker evidence before every socket attempt. Breaker-authority or outbox read/write failure admits 0 attempts; reserved next tuple survives restart. |
| new **T6-W12** | A6.20 provider/live half | 1 phase | external provider adapter, cost/burn correlator, recovery-horizon tests and exact live evidence | T6-W15 · T1-W6 · T3-W16 · O-CFINVENTORY · T7-W4b | W3 adds fully paginated provider/correlator proof to the already deployed base, then proves the 120 s push/source-failure, 240 s one-scan and 330 s two-scan SLOs plus all-source-high-water/330 s recovery horizon. `provider.test.ts`, `correlator.test.ts`, `incident-boundary.test.ts` and `recovery-horizon.test.ts` are mandatory. It cannot run without T6-W15 or alone seal A6.20; later T6-W10 consumes the completed primitive. |
...
| new **T6-W14** | A6.22 | 1 | DO-hook lifecycle authority + read-only outer route + authenticated canary sampler/outbox/validation/re-enable + exact live evidence | T1-W6 · T6-W12 · T6-W13 · O-CANARY · O-MONITORHOST | Every 60 s the canary sampler validates the DO-authored lifecycle record, durably enqueues the exact envelope, and sends it to the external base. Outer Worker and sampler cannot author/refresh lifecycle state. Ordered byte-identical retry, nonce/freshness/anti-replay/disconnect refusal, external missing-sample page and zero container usage precede non-waking re-enable. |
