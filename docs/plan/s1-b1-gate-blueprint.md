...
Run these against the frozen tip, in parallel where scopes are disjoint. Stop
before heavy CI if a cheap structural check proves the tip is not complete.
...
2. **Cheap gates (seconds to a few minutes).** Run
   ...
   and DCO/workflow linters required by the final tip. Run each once;
...
3. **Focused scope signals (parallel).**
   ...
   - Canary/alerts/T6-W4/T6-W9/T6-W13: `npm ci` once in
     `deploy/cloudflare-canary`, then `npx vitest run` with the scheduled-tick,
   ...
   - Worker/DevEnv/T9-W1: `npm ci`, `npm run typecheck`, then one focused
     `npx vitest run <files>` in `deploy/cloudflare`; use coverage only in the
     required worker gate, not in every focused invocation.
   - T6-W2/moat: run the two workflow-level planted-miss checks and
...
4. **Complete B1 matrix, only after all twelve scopes and DAG edges are
   ...
   scan lanes. Run applicable image/build checks only when their changed paths
   require them. GitHub `startup_failure`/zero jobs is `BLOCKED`, not evidence.
...
moat jobs. The workflow timeouts are 10–30 minutes per job, but cold Rust
...
execution can consume 6–10 hours. This is an estimate, not a passed SLA.

## Deduplication rules

`cargo test --workspace --locked` is the Rust correctness source of truth;
package tests must not be repeated as separate WP-wide suites after it passes.
...
The worker workflow owns the single coverage run; focused Vitest runs are
...
executed on the identical final SHA is reused by manifest hash, not rerun.
...
- After a fix, run its focused check, then the affected workflow job. Reopen
