...
The added tests for the clw helper covered an empty miss, malformed AC record, cached nonzero replay without spawn, and CAS bytes substituted under the recorded digest. Existing tests at the sibling HEAD covered default miss execution and ordinary hit replay. `cargo fmt --all -- --check`, `git diff --check`, and `git apply --check` passed in the disposable authoring clone. No build, dependency install, CI, provider call, credential read, or release work was performed.
