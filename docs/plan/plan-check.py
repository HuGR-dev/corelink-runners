...
B = {}
def b(name, ids): B[name] = ids.split()
...
print(f"findings: {len(known)}   assigned-unique: {len(set(assigned))}")
for name, ids in B.items():
    print(f"  {name:22s} {len(ids):3d}")
print(f"\nDUPLICATE (owned twice): {len(dupes)}")
for i in sorted(dupes): print("   ", i, "x", dupes[i], "in", [n for n,v in B.items() if i in v])
print(f"\nUNKNOWN id (typo / not a finding): {len(unknown)}")
for i in unknown: print("   ", i)
print(f"\nORPHAN (no bucket): {len(orphans)}")
for i in orphans: print("   ", i)

ok = not dupes and not unknown and not orphans
