...
The package evidence touches `corelink-workspaces/docs/validation/2026-09-06-t6-w2-acceptance.md`; the guard stack changes that repository's release workflow and scripts. Neither path belongs in the Runner product tree, so this record leaves both stacks in the workspace repository.
...
3. Execute the shared B1 gate against the composed Runner candidate and the pinned
...
Runner bundle merges: the workspace promotion remains a separate repository action,
and the Runner evidence feeds B1 only.
