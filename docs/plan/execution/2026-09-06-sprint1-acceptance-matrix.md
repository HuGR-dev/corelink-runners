...
**Fonte de estado:** `docs/plan/delivery-ledger.json` no tip de integração
`2a3c49e773e47faa2715bf6aeb1f5a74cf2bed51`
...
| `implemented` (partial) | 2 | T6-W15, T9-W1 |
| `blocked` (inventory) | 2 | T6-W13, T2-W3 |
...
`ambiguous_missing_inventory`. T6-W13 e T2-W3 continuam explicitamente
...
faltante. T6-W15 e T9-W1 têm implementação parcial registrada e seguem como
`implemented`, sem serem promovidos a aceite.

## Matriz operacional

| WP | Status real | SHA / fonte registrada | Lacuna concreta para aceite | Check focado mínimo para dispatch | Dono recomendado |
|---|---|---|---|---|---|
| **T3-W18** | `acceptance-ready` | `bb165bad54125460ebae213e405df3a75baa6566`; `2d2930ac7fb0ebdcc5c76291fa12f94abb586dae` · `T3-W18-containment-live.json`, `2026-09-06-sprint1-source-closure.json` | Executar matriz production version-bound de três estados **10/10**, resume ordenado não vazio, e observar ausência de efeitos indevidos e instâncias em execução. O artefato atual continua RED nesses pontos. | Conferir versão/config digest do alvo; rodar somente o driver 10/10 + resume não vazio; arquivar respostas, efeitos e contagem de instâncias. | Sol / production-risk |
...
| **T6-W4** | `acceptance-ready` | `bb165bad54125460ebae213e405df3a75baa6566`; `f77e4d03fa0db053522bd6571a9ef5286099a130`; `f8270299f581e9eefa7274bfcf1a0304acb2a8f4`; `e1501f6246fba66d6deaa9aad9ddd4be0be71586` · envelope, source-review, stress e secret-scan evidence | Falta apenas o aceite de composição: config do owner e evidência production/named-host; o produtor e seus checks focados já estão aprovados. | Exercitar uma sequência capacity-1 enqueue → ACK/terminal, incluindo retry/crash boundary e flag inválida; confirmar o envelope de evidência e o host nomeado. | Sol / production-risk |
| **T6-W9** | `acceptance-ready` | `bb165bad54125460ebae213e405df3a75baa6566`; `04743acf9a7a7f2385ccce1ebd13882530b7d4f1`; `47ae1bc370c6a68f79aa126e203ecf661b59415a` · `2026-09-05-rule-matrix-and-billing-review.json` | Faltam binding externo real, detector implantado e aceite end-to-end C1–C5; não há crédito production registrado. | Validar a tabela C1–C5 em teste focado (positivo, malformed/auth e resposta drenada) e confirmar o binding/detector version-bound antes do probe production. | Sol / alerting |
...
| **T6-W15** | `implemented` (partial) | Implementação em andamento no worktree dedicado `/private/tmp/corelink-t6-w15-owner-20260906`; contratos: `2026-09-06-techlead-takeover.md`, `2026-09-06-f008-credential-lifecycle-integration.md` | O WP não está bloqueado pelos sete dias de A6.17: essa observação é pós-entrega e não impede merge/go-production. Antes do aceite base ainda faltam trusted time/TSA, WORM, independência de três contas, crash/rotação/isolamento e o probe A6.10 3/3; manter UNKNOWN fail-closed. | Check focado de implementação/contrato: verificar o runtime principal, signer registry, ACK recovery, page ACK e codecs integrados; validar A6.10 3/3 e registrar as qualificações. Os sete dias entram depois como evidência operacional pós-entrega. | Sol / alerting + owner do monitorhost |
...
| **T9-W1** | `implemented` (partial) | Implementação parcial integrada; decisão/autoridade D2: `docs/plan/execution/2026-09-06-t9-w1-d2-authority-decision.md`; SHAs de implementação permanecem os registrados no ledger | A decisão D2 está integrada; não há mais bloqueio de decisão/inventário. O WP continua parcial: faltam autoridade qualificada de terminal/cancelamento e uso real F005/F007, producers de hard-stop/usage, baseline externo reconciliado, chaves públicas de produção, migração pareada e aceite production. A reserva de 8h não prova hard-stop. | Validar a autoridade e os paths integrados, depois rodar apenas a regressão compute-budget (bearer válido e 503 sem settlement) e registrar baseline/chaves; manter `partial` até os produtores e production acceptance fecharem. | Sol / decision-gated + owner do contrato server |
...
4. Manter T2-W3 e T6-W13 fora da fila de aceite até o inventário ser
   materialmente qualificado. T6-W15 e T9-W1 podem continuar em execução como
   `implemented (partial)`, mas não são promovidos a `ACCEPT` até as lacunas
   próprias fecharem. Um `prepared`, `partial` ou SHA listado sem
   ancestralidade não é promoção.
