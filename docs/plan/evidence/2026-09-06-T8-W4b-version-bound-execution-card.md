...
This card is prepared for one read-only probe after T3 provisions an ephemeral
runner. It does not authorize deployment or change any Worker/container state.

The deploy-bound runner image is:
...
The runner bootstrap is version-bound by `deploy/runner/Dockerfile`:
...
When the runner is online, set `CONTAINER_APP_ID` to the exact Cloudflare
RunnerContainer application id and run:
...
then inspects the probe run log for the process witness and forbidden JIT
material. Any image drift, absent instance, leaked bridge name, or missing
witness fails closed. T3-W18 remains the only live dependency; this card is
ready to execute as soon as its runner is available.
