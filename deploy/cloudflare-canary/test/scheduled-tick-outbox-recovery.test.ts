...
const config: TickConfig = {
  ...
  monitorRearmTupleDigest: "a".repeat(64), envelopeHmacKey: "envelope",
  trustedNow: () => 1_000,
};
...
describe("scheduled tick durable outbox", () => {
  ...
  it("bounds a hung response body at the original 60-second deadline", async () => {
    ...
    const outbox = new CanaryTickOutbox(state());
    const fetcher = vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(new ReadableStream({ start() { /* deliberately never close */ } }), { status: 200 }));
    const pending = outbox.enqueueAndDrain(config, 1_000);
    await vi.waitFor(() => expect(fetcher).toHaveBeenCalled());
    await vi.advanceTimersByTimeAsync(60_000);
    await expect(pending).resolves.toContain("deadline");
  ...
  it("never authorizes an ACK observed at the deadline", async () => {
    ...
    const outbox = new CanaryTickOutbox(state());
    const dynamic = vi.spyOn(globalThis, "fetch").mockImplementation(async (_url, init) => {
      const envelope = JSON.parse(String(init?.body));
      const signed = await import("./scheduled-tick-fixtures").then(({ signedToken, ACK_FIELDS }) => signedToken({
        ack_version: "1", event_id: envelope.event_id, producer_seq: envelope.producer_seq, payload_digest: envelope.payload_digest,
        source: envelope.source, service: envelope.service, application: envelope.application, key_id: envelope.key_id,
        credential_epoch: envelope.credential_epoch, monitor_rearm_tuple_digest: envelope.monitor_rearm_tuple_digest,
        ingest_commit_id: "commit", committed_at: 1_001, signer_key_id: "ack-signer", signer_epoch: "4",
      }, ACK_FIELDS, "ack-signer", "4"));
      config.ackVerifier = signed.verifier;
      return new Response(JSON.stringify(signed.token), { status: 200 });
    });
    config.trustedNow = () => trustedNow;
    expect(await outbox.enqueueAndDrain(config, 1_000)).toContain("invalid ACK");
  ...
  it("bounds a verifier that never resolves at the same original deadline", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(1_000);
    const verifier = { verify: async () => await new Promise<"valid">(() => undefined) };
    const boundedConfig = { ...config, ackVerifier: verifier, trustedNow: () => 1_001 };
    const outbox = new CanaryTickOutbox(state());
    const fetcher = vi.spyOn(globalThis, "fetch").mockImplementation(async (_url, init) => {
      const e = JSON.parse(String(init?.body));
      return new Response(JSON.stringify({ ack_version: "1", event_id: e.event_id, producer_seq: e.producer_seq, payload_digest: e.payload_digest, source: e.source, service: e.service, application: e.application, key_id: e.key_id, credential_epoch: e.credential_epoch, monitor_rearm_tuple_digest: e.monitor_rearm_tuple_digest, ingest_commit_id: "commit", committed_at: 1_001, signer_key_id: "ack-signer", signer_epoch: "4", signature: "fixture" }), { status: 200 });
    });
    const pending = outbox.enqueueAndDrain(boundedConfig, 1_000);
    await vi.waitFor(() => expect(fetcher).toHaveBeenCalled());
    const outcome = Promise.race([pending, new Promise<string>((resolve) => setTimeout(() => resolve("timeout"), 60_001))]);
  ...
  it("reschedules an early alarm and closes the hole at the original deadline", async () => {
    ...
    const storage = {
      get: async <T>(key: string) => values.get(key) as T | undefined,
      put: async (key: string, value: unknown) => { values.set(key, value); },
      setAlarm: async (at: number) => { alarms.push(at); }, deleteAlarm: async () => undefined,
    };
    ...
    await outbox.alarm();
    expect((await storage.get<{ head?: unknown; terminal?: string }>("state"))?.terminal).toBe("TIMED_OUT");
    vi.restoreAllMocks();
    vi.useRealTimers();
  });
});
