...
describe("scheduled tick ACK recovery gate", () => {
  it("accepts a real recovery signature bound to SHA-256(canonical ACK + signature)", async () => {
    ...
    expect(await validRecovery(recovery.token, head, original.token, recovery.verifier)).toBe(true);
  });
  it.each([
    ...
    const original = await signedAck({ signer_key_id: "revoked" });
    const recovery = await signedRecovery(original.token);
    expect(await validRecovery({ ...recovery.token, ...mutation }, head, original.token, recovery.verifier)).toBe(false);
  });
  it("rejects a late recovery, wrong verifier, and altered original ACK", async () => {
    const original = await signedAck({ signer_key_id: "revoked" });
    const recovery = await signedRecovery(original.token);
    expect(await validRecovery(recovery.token, head, original.token, undefined)).toBe(false);
    const altered = { ...original.token, ingest_commit_id: "other" };
    expect(await validRecovery(recovery.token, head, altered, recovery.verifier)).toBe(false);
  });
});
