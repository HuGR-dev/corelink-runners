...
export async function signedToken<T extends object>(
  unsigned: T,
...
export async function signedAck(overrides: Partial<typeof ackUnsigned> = {}) {
  return signedToken(
    { ...ackUnsigned, ...overrides },
    ACK_FIELDS,
    "ack-signer",
    "4",
  );
}
export async function signedRecovery(
  ack: AckToken,
  overrides: Partial<Omit<AckRecovery, "signature">> = {},
) {
  const originalAckDigest = await sha256(
    JSON.stringify([...JSON.parse(canonical(ack, ACK_FIELDS)), ack.signature]),
  );
  const unsigned: Omit<AckRecovery, "signature"> = {
    ...
    key_id: envelope.key_id,
    credential_epoch: envelope.credential_epoch,
    original_monitor_rearm_tuple_digest: envelope.monitor_rearm_tuple_digest,
    ingest_commit_id: ack.ingest_commit_id,
    original_ack_digest: originalAckDigest,
    revocation_record_digest: HEX,
    signer_rotation_manifest_digest: HEX,
    signer_manifest_generation: 2,
    signer_manifest_witness_root_digest: HEX,
    current_monitor_rearm_tuple_digest: envelope.monitor_rearm_tuple_digest,
    recovery_signer_key_id: "recovery",
    recovery_signer_epoch: "5",
    issued_at: 1_010,
    ...overrides,
  };
