...
export function parseProbeFlag(value: string | undefined): ProbeFlag {
  ...
  return {
    valid: false,
    reason: "FABRIC_PROBES_ENABLED must be exactly string 0 or 1",
  };
}
...
export function tickConfig(env: {
  CANARY_TICK_INGEST_URL?: string;
  CANARY_TICK_SOURCE?: string;
  CANARY_TICK_SERVICE?: string;
  CANARY_TICK_APPLICATION?: string;
  CANARY_TICK_KEY_ID?: string;
  CANARY_TICK_CREDENTIAL_EPOCH?: string;
  CANARY_TICK_MONITOR_REARM_TUPLE_DIGEST?: string;
  CANARY_TICK_ENVELOPE_HMAC_KEY?: string;
}): TickConfig | null {
  const fields = [
    "CANARY_TICK_INGEST_URL",
    "CANARY_TICK_SOURCE",
    "CANARY_TICK_SERVICE",
    "CANARY_TICK_APPLICATION",
    "CANARY_TICK_KEY_ID",
    "CANARY_TICK_CREDENTIAL_EPOCH",
    "CANARY_TICK_MONITOR_REARM_TUPLE_DIGEST",
    "CANARY_TICK_ENVELOPE_HMAC_KEY",
  ] as const;
  if (fields.some((field) => !env[field])) return null;
  return {
    ingestUrl: env.CANARY_TICK_INGEST_URL!,
    source: env.CANARY_TICK_SOURCE!,
    service: env.CANARY_TICK_SERVICE!,
    application: env.CANARY_TICK_APPLICATION!,
    keyId: env.CANARY_TICK_KEY_ID!,
    credentialEpoch: env.CANARY_TICK_CREDENTIAL_EPOCH!,
