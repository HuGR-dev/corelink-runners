...
function signer(role: "journal" | "witness") {
...
function harness() {
  ...
  let failNext = false;
  const journal = { append: async (record: any) => { if (failNext) { failNext = false; throw new Error("worm unavailable"); } const key = `${record.sequence}/${record.operationId}`; const prior = objects.get(key); if (prior) return prior; const receipt = { operationId: record.operationId, sequence: record.sequence, recordDigest: digest(JSON.stringify(record)), previousDigest: record.previousDigest, bucket: "b", key, versionId: "v1", retainedUntilMs: now + 8 * 24 * 60 * 60 * 1000 }; objects.set(key, receipt); return receipt; } };
  ...
  return { witness, journalSigner, store, failWorm: () => { failNext = true; } };
}
...
describe("DurableCheckpointWitness", () => {
  ...
  it("rejects gaps, forks, and wrong journal signatures before WORM", async () => {
    ...
    const other = signer("journal"); const forged = await checkpoint(other);
