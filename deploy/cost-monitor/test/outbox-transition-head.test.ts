...
import { canonicalJson } from "../src/types.js";
...
function make(store: MemoryStateStore, now: number, lifecycle = false) {
  const audit = { append: async (operationId: string) => receipt(operationId), verify: async () => {} };
  return new IngestService({ store, audit, clock: { now: async () => ({ timeMs: now, proofDigest: "a", requestDigest: "b", authority: "test" }) }, signingAuthority: { resolveIngestSigner: async () => signer, isIngestSignerCurrent: async () => true }, secrets: { load: async () => secret }, registrations: [lifecycle ? lifecycleRegistration : registration], namespace: "n", monitorTupleDigest: "b".repeat(64), destination: "ops" });
}
