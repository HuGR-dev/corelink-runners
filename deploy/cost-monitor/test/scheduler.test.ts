...
const registration: SourceRegistration = { source: "s", service: "svc", application: "app", keyId: "k", credentialEpoch: "1", secretArn: "arn", allowedKinds: ["canary-tick"], intervalMs: 300_000, sourceVersion: "v1", authoritySourceId: null, enabled: true };
const digest = "a".repeat(64);
const cursor = (overrides: Partial<SourceCursor> = {}): SourceCursor => ({ source: "s", service: "svc", application: "app", keyId: "k", credentialEpoch: "1", lastSequence: 2, lastEnvelopeDigest: digest, lastOccurredAt: 1_000, lastScheduledFor: 1_000, firstAcceptedAt: 1_000, lastAcceptedAt: 1_000, expectedAt: 1_300_000, quarantined: false, lifecycle: null, lastLifecycleNonce: null, sourceHealth: "healthy", sourceReason: "healthy", ...overrides });
