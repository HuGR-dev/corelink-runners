...
function storage() {
  const map = new Map<string, unknown>(); let queue = Promise.resolve(); let fail = false;
  ...
  const result = { ...base, map, failNext: () => { fail = true; }, transaction: async <T>(fn: (s: typeof base) => Promise<T>) => {
    const run = queue.then(async () => {
      ...
      const value = await fn(tx); if (fail) { fail = false; throw new Error("injected rollback"); }
      map.clear(); for (const [k, v] of snapshot) map.set(k, copy(v)); return value;
    }); queue = run.then(() => undefined, () => undefined); return run;
  } };
  return result as unknown as AuthorityStorage & { failNext(): void; map: Map<string, unknown> };
}
