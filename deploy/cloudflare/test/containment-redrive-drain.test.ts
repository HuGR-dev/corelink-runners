...
const observeQueuedJob = async () => ({ httpStatus: 200, job: { status: "queued" } });
...
describe("T3-W17 deterministic continuation crash seams", () => {
  ...
  it("leaves durable state at every named drain crash boundary", async () => {
    {
      ...
      await expect(runContainmentDrain(envWithAuthority(d, store, authority), { observeTerminalJob: observeQueuedJob })).rejects.toThrow("before claim");
    ...
    }
    {
      ...
      await runContainmentDrain(envWithAuthority(d, store, authority), { observeTerminalJob: observeQueuedJob, driveSpawn: drive });
    ...
    {
      const { d, store } = await queuedDrain();
      await runContainmentDrain(env(d, store), { observeTerminalJob: observeQueuedJob, claimSpawn: async () => { throw new Error("after permit"); }, driveSpawn: async () => providerReceipt({ jobId: "1", repo: "acme/repo" }) });
      expect((await d.instance.getEvent("evt-1"))).toMatchObject({ state: "CLAIMED", effect_permit: null });
    ...
    }
    {
      ...
      await runContainmentDrain(env(d, store), { observeTerminalJob: observeQueuedJob, claimSpawn: claim, bindContainmentSpawnClaim: bind, driveSpawn: async () => providerReceipt({ jobId: "1", repo: "acme/repo" }) });
    ...
    }
    {
      ...
      await runContainmentDrain(env(d, store), { observeTerminalJob: observeQueuedJob, claimSpawn: () => realClaimSpawn(store, "1"), bindContainmentSpawnClaim: bind, driveSpawn: drive });
    ...
    }
    {
      ...
      await runContainmentDrain(envWithAuthority(d, store, authority), { observeTerminalJob: observeQueuedJob, claimSpawn: () => realClaimSpawn(store, "1"), bindContainmentSpawnClaim: async () => {}, driveSpawn: drive });
    ...
    }
    {
      ...
      await runContainmentDrain(envWithAuthority(d, store, authority), { observeTerminalJob: observeQueuedJob, claimSpawn: () => realClaimSpawn(store, "1"), bindContainmentSpawnClaim: async () => {}, driveSpawn: drive as never });
  ...
  it("allows only the deferred old permit holder into the continuation after lease reclaim", async () => {
    ...
    const oldRun = runContainmentDrain(env(d, store), {
      ...
      claimSpawn: () => realClaimSpawn(store, "1"),
    ...
    await runContainmentDrain(env(d, store), { observeTerminalJob: observeQueuedJob, claimSpawn: () => realClaimSpawn(store, "1"), bindContainmentSpawnClaim: async () => {}, driveSpawn: enteredNew });
  ...
  it("reclaims PREPARED and CLAIM_ACQUIRED owners before the sole drain permit", async () => {
    for (const crashState of ["PREPARED", "CLAIM_ACQUIRED"] as const) {
      ...
      await runContainmentDrain(envWithAuthority(d, store, oldAuthority), { observeTerminalJob: observeQueuedJob, driveSpawn: async () => providerReceipt({ jobId: "1", repo: "acme/repo" }) });
  ...
  it("retries safely when the external claim is refused after predecessor reap", async () => {
    ...
    await runContainmentDrain(env(d, store), { observeTerminalJob: observeQueuedJob, claimSpawn: async () => false, driveSpawn: refusedDrive });
  ...
  it("recovers a proven permit through the real drain without a second continuation entry", async () => {
    ...
    vi.setSystemTime(T0 + 120_000);
    await runContainmentDrain(env(d, store), { observeTerminalJob: observeQueuedJob, claimSpawn: claim, bindContainmentSpawnClaim: bind, driveSpawn: drive });
