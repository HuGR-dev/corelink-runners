...
async function response(url: string, state = url.endsWith("/reserve") ? "prepared" : url.endsWith("/cancel") ? "cancelled" : "active") {
  const terminal = state === "cancelled" || state === "settled";
  return terminal ? terminalResponse(state as "cancelled" | "settled", sessionUuid) : new Response(JSON.stringify({ reservation_id: sessionUuid, state }), { status: 200 });
}
async function responseFor(id: string, state: string) {
  const terminal = state === "cancelled" || state === "settled";
...
describe("authorized DevEnv compute composition", () => {
  ...
  it("retries failures from an earlier page after a restart and a successful tail", async () => {
    ...
    for (let n = 1; n <= 26; n++) {
      ...
      await f.ctx.storage.put(`compute:obligation:${id}`, {
        ...
        ...(n === 1 ? {} : await cancelledEvidence(id)),
      });
    }
  ...
  it("carries the drain cursor across restart before reaching an expired preparation", async () => {
    ...
    for (let n = 1; n <= 25; n++) {
      const id = `11111111-1111-4111-8111-${String(n).padStart(12, "0")}`;
      await f.ctx.storage.put(`compute:obligation:${id}`, { binding: { token: obligationToken(id, id), reservationId: id, tenantId, workloadKind: "devenv", workloadId: id, vcpuCount: 4, maximumWallMs: 28_800_000 }, phase: "terminal", deadlineMs: NOW + 60_000, ...(await cancelledEvidence(id)) });
    }
