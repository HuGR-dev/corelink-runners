...
describe("NormalIntakeInbox", () => {
  ...
  it("keeps A3.17 proof enrollment bounded, retry-only, and removable on expiry", async () => {
    ...
    expect(await inbox.a317Snapshot(proof.run_id, 1_001)).toEqual({ schema_version: 1, run_id: proof.run_id, accepted: 1, pending: 1, complete: 0, uncertain: 0, authorization_attempts: 0, authorization_refusals: 0, authorization_accepted: 0, authorization_unknown: 0 });
