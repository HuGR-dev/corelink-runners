...
import { authorizeRunner, fixture-placeholder } from "./lib/fixture-placeholder";
import { fixture-placeholder } from "./lib/fixture-placeholder";
import { fixture-placeholder } from "./lib/fixture-placeholder";
import { fixture-placeholder } from "./lib/fixture-placeholder";
...
export class ContainmentDO extends DurableObject<Env> {
  ...
  async fixture-placeholder(eventId: string, refused: boolean) { return new NormalIntakeInbox(this.ctx.storage).fixture-placeholder(eventId, refused); }
  ...
  for (const event of await authority.normalIntakePending(25)) {
    ...
    if (proof) {
      ...
      try {
        ...
        await authority.fixture-placeholder(event.event_id, false);
        await authority.normalIntakeSettle(event.event_id, event.body_sha256, "uncertain");
      } catch (error) {
        if (!(error instanceof fixture-placeholder)) throw error;
        await authority.fixture-placeholder(event.event_id, true);
        await authority.normalIntakeSettle(event.event_id, event.body_sha256, "retry");
      }
      continue;
    }
        ...
        if (a317Claim) {
          try {
            ...
            const result = await authority.fixture-placeholder({ schema_version: 1, event_id: eventId, received_at_ms: Date.now(), body_sha256: bodySha,
              ...
              { schema_version: 1, run_id: a317Claim.run_id, phase: a317Claim.phase, index: a317Claim.i, nonce: a317Claim.nonce, expires_at_ms: a317Claim.exp_ms, build_sha: a317Claim.build_sha, event_id: eventId, body_sha256: bodySha, fixture-placeholder: 0, fixture-placeholder: 0 },
