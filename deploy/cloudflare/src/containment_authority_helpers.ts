...
export function isValidJobIndex(value: unknown, repo: string, jobId: string): value is ContainmentJobIndex {
  ...
  return r.schema_version === 1 && r.repo === repo && r.job_id === jobId && Array.isArray(r.active_event_ids) && r.active_event_ids.length <= MAX_ACTIVE_INDEX_EVENTS
    && r.active_count === r.active_event_ids.length && Number.isFinite(r.updated_at_ms)
    && new Set(r.active_event_ids).size === r.active_event_ids.length && r.active_event_ids.every((id) => typeof id === "string" && id.length > 0);
}
...
export function canonicalSafeJobId(jobId: unknown): string | null {
  if (typeof jobId !== "string" || !/^[1-9][0-9]*$/.test(jobId)) return null;
  try { const numeric = BigInt(jobId); return numeric > BigInt(Number.MAX_SAFE_INTEGER) ? null : numeric.toString(10); } catch { return null; }
}
