...
MODE=plan ACK_ARG='' ROOT='' COMMIT='' VERSION='' APP_ID='' DIGEST='' RECOVER_INTROSPECT=0
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --recover-introspect) RECOVER_INTROSPECT=1; shift;; --introspect-pat-file) PAT_FILE="${2:?}"; shift 2;;
...
[ "$RECOVER_INTROSPECT" != 1 ] || [ -n "$NEW_INTROSPECT_KEY_FILE" ] || die 'recovery requires --new-introspect-key-file'
...
[ "$RECOVER_INTROSPECT" != 1 ] || { [ -e "$NEW_INTROSPECT_KEY_FILE" ] && [ ! -L "$NEW_INTROSPECT_KEY_FILE" ] || die 'new introspect key file is missing'; }
[ "$RECOVER_INTROSPECT" != 1 ] || single_line_file "$NEW_INTROSPECT_KEY_FILE" || die 'new introspect key must be owner-only regular 0600 single-line file'
