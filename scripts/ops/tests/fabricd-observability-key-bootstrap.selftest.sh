...
run_resume_artifact_mismatch() {
  ...
  jq -n --arg commit "$commit" --arg digest "$MOCK_DIGEST" --arg version "$recovery_version" --arg app "$recovery_app" '{schema_version:"evidence/v1",artifact_id:"fabricd-observability-key-bootstrap-introspect-recovery-failure",status:"RED",operation_mode:"introspect-recovery",source:{repository:"corelink-runners",commit_sha:$commit},provider:{worker:"corelink-fabricd",expected_version:$version,expected_image_digest:$digest,app_id:$app},phase:"container-recreate",secrets:{FABRIC_INTROSPECT_KEY_put_completed:true,FABRIC_OBSERVABILITY_KEY_put_completed:true},gates:{refreeze_result:"green"},outcome:"RED",rerun_guard:"armed"}' | jq "$artifact_filter" > "$oob/fabricd-observability-key-bootstrap-introspect-recovery-failure.json"
