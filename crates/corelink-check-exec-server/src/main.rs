...
//! argv) unless a mode-0400 auth-token file is mounted. The path is selected by
//! `EXEC_SERVER_AUTH_TOKEN_FILE` (default:
//! `/run/secrets/corelink-check-exec-server-auth-token`). The legacy
//! value-bearing `EXEC_SERVER_AUTH_TOKEN` environment variable is refused. The
//! unauthenticated posture (container boundary + Worker bearer as the only gates)
//! remains available ONLY behind the explicit `CHECK_EXEC_ALLOW_UNAUTH` opt-in.
...
async fn main() -> anyhow_lite::Result {
    // AU1.9 + WP-9c: production boot reads only a mode-0400 secret file. The
    // library owns both the file validation and the fail-closed opt-in rule, so
    // alternate binary entry points cannot silently re-open /exec.
    let auth = ExecAuth::from_secret_file_env().map_err(|e| e.to_string())?;
