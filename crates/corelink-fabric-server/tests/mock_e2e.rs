...
//! contract (external consumer handoff `docs/handoff/2026-06-13-fabric-
...
//! external consumers pre-build their offline adapters against this frozen output and
//! against the REAL signed attestation path. Drift in `MOCK_STDOUT` or in the
//! mock exec / attestation path breaks an external consumer's pre-build — and it must
...
use corelink_fabric_server::{
    AppState, HookRegistry, MOCK_STDOUT, MockLeasedExec, StaticPlans, StaticTokenStore,
    SystemClock, app_full, verify_execution,
};
...
/// The FROZEN sha256 of [`MOCK_STDOUT`], pinned by external offline adapters
...
/// drifts — before it can break an external consumer's pre-build.
#[tokio::test]
async fn mock_exec_e2e_pins_consumer_contract() {
    ...
    assert_eq!(
        ...
        "the mock stdout content-ref must equal the FROZEN sha256 \
         {FROZEN_STDOUT_SHA256} — this is the external consumer-contract pin"
    );
...
/// the attestation HALF of the consumer contract: external consumers verify the mock's
