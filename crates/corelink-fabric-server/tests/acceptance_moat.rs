...
async fn a6b_acquire_injects_per_job_pat_into_box_env_not_tenant_pat() {
    ...
    assert_eq!(captured.len(), 1, "A6b: exactly one box must be provisioned");
...
async fn a7_mint_failure_fails_closed_no_box() {
    ...
    impl CasPatMint for FailingMint {
        ...
        ) -> Pin<Box<dyn std::future::Future<Output = std::result::Result<MintedPat, MintError>> + Send + 'a>>
        {
    ...
    let result = fail_mint
        .mint("acme", "job-fail", 9_999_999_999_999)
        .await;
...
async fn a8_clw_drive_exit_code_transparent_and_written_back_on_success() {
    ...
    let outcome = driver.drive("lease-abc").await.expect("A8: drive must not error");
...
async fn a8_nonzero_child_exit_is_transparent_and_not_cached() {
    ...
    let outcome = driver.drive("lease-fail").await.expect("A8: drive must not error");
...
async fn a8_clw_internal_exit_is_distinct_from_child_exit() {
    ...
    assert!(
        matches!(clw_outcome, ClwDriveOutcome::ClwFailed { clw_exit_code: 2, .. }),
